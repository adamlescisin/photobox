    ...
    <div className="max-w-lg space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} className={inputClasses} placeholder="Maturitní ples 2026" required />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} className={`${inputClasses} resize-none`} rows={3} placeholder="Krátký popis akce…" />
        </div>

        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <Label htmlFor="pw-toggle">Ochrana heslem</Label>
            <Switch id="pw-toggle" checked={passwordEnabled} onCheckedChange={setPasswordEnabled} />
          </div>
          {passwordEnabled && (
            <input
              type="password"
              ...
              placeholder="Heslo pro přístup ke galerii"
              required={passwordEnabled}
            />
          )}
        </div>

        <button type="submit" disabled={loading} className="w-full rounded-xl gradient-accent py-3 text-sm font-semibold text-primary-foreground shadow-glow transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-50">
